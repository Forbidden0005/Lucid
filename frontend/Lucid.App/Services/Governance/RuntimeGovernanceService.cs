...
///
/// Battery check:
///   PowerManager.BatteryStatus is read on each evaluation. The WinRT call is
...
public sealed class RuntimeGovernanceService : IRuntimeGovernanceService, IDisposable
{
    ...
    {
        // Battery status via WinRT — cheap cached read, safe off-thread.
        bool isOnBattery = false;
        try
        {
            isOnBattery = PowerManager.BatteryStatus == BatteryStatus.Discharging;
        }
        catch { /* WinRT not available in some sandboxed contexts — treat as plugged in */ }

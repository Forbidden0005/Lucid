...
public static class ProcessIdentityValidator
{
    ...
    public static VerifiedProcessTarget? TryVerify(int pid, string? expectedName)
    {
        ...
        {
            // Unexpected (e.g. UnauthorizedAccessException on restricted processes).
            return null;
        }
    }

    /// <summary>
    /// Captures a fingerprint for diagnostics/audit purposes.
    /// Returns null if the process cannot be queried.
    /// </summary>
    public static ProcessExecutionFingerprint? CaptureFingerprint(int pid)
    {
        var target = TryVerify(pid, expectedName: null);
        if (target is null) return null;

        return new ProcessExecutionFingerprint(
            ProcessId:        pid,
            ActualProcessName: target.VerifiedName,
            MainModulePath:   target.MainModulePath,

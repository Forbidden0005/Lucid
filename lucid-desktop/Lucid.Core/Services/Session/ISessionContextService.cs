...
///   Session start      — time Lucid was launched (user-session proxy).
...
public interface ISessionContextService
{
    ...
    /// Call once from the composition root, after both services have started.
    /// </summary>
    void Start();

    /// <summary>
    /// Unsubscribes from all upstream services.
    /// Call from the composition root shutdown path.
    /// </summary>
    void Stop();
}
